...
from languages.serializers import LanguageSerializer, CountryLanguageSerializer
...
class CountryLanguageViewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, pk):
        language = get_object_or_404(CountryLanguage.objects.filter(pk=pk))
        serializer = CountryLanguageSerializer(language, many=False)
        return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def destroy(self, request, pk=None):
        country_language = get_object_or_404(CountryLanguage.objects.filter(pk=pk))
        country_language.status = 3
        country_language.save()
        return Response({'deleted': country_language.title}, status=status.HTTP_202_ACCEPTED)

    @action(detail=False, methods=['GET'], name='Default country languages registration', url_name='set-default-country-languages')
    def set_default_country_languages(self, request):
        from languages.signals import create_default_country_languages
        try:
            create_default_country_languages()
            return Response({'default_country_languages_installed': True}, status=status.HTTP_201_CREATED)
        except Exception as e:
            print(e)
            return Response({'default_country_languages_installed': False}, status=status.HTTP_406_NOT_ACCEPTABLE)
